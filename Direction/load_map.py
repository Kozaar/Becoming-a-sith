...
def load_map():
    ...
    current_location = session.query(
        Locations).filter(Locations.id == 1).first()
    print('Текущая локация: ', current_location.name)
    print(f'Описание: {current_location.description} \n')
    # print('Куда я могу пойти:')

    # up_location = session.query(Locations).filter(
    #     Locations.id == current_location.linkUp).first()
    # if up_location:
    #     print('Вверх: ', up_location.name)
    # down_location = session.query(Locations).filter(
    #     Locations.id == current_location.linkDown).first()
    # if down_location:
    #     print('Вниз: ', down_location.name)
    # left_location = session.query(Locations).filter(
    #     Locations.id == current_location.linkLeft).first()
    # if left_location:
    #     print('Влево: ', left_location.name)
    # right_location = session.query(Locations).filter(
    #     Locations.id == current_location.linkRight).first()
    # if right_location:
    #     print('Вправо: ', right_location.name)

    # print('-----------------------------')
    # print('Я иду вверх')
    # current_location = session.query(Locations).filter(
    #     Locations.id == current_location.linkUp).first()
    # print('Текущая локация: ', current_location.name)
